...
model = @model begin
  @param begin
    tvKa ∈ RealDomain(; lower=0.)
    NN ∈ MLP(4, 4, 4, (1, identity); reg=L2(1.))
...
covariate_pred = predict(covariate_model, testpop, p_covs; obstimes);
...
deep_pred = predict(covariate_model, testpop, coef(deep_fpm); obstimes);
